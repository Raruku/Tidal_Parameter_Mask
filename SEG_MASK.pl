...
#This script generates a segmentation mask for both the shallow and deep galaxies. Deep masks are applied to the shallow images
		...
		#	FOR ALL THE CRAP!
			foreach my $sexCount (0 .. scalar @N - 1) {
				...
				if ($x_1 >= $X[$sexCount]  && $y_1 >= $Y[$sexCount] && $y_2 <= $Y[$sexCount] && $x_2 <= $X[$sexCount]) {
					...
					#hack so DR7 gets all the right masks
